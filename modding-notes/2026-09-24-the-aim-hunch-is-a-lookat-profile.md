...
**Flashlight clip** (`track.py`: share of the left-eye view that is red jacket, every 0.1 s; the collar tip hides behind the flashlight hand, so area is the ruler here) `[measured 2026-09-24, n=5 presses]`: presses at 9.1, 12.1, 15.0, 18.0, 20.8 s - jacket **~7% -> ~10.5% of the view within 0.1-0.2 s**, creeping up a little more while held (12.2->13.8 s: 10.4 -> 11.4%); releases fall back over ~0.4 s. No-flashlight clip by the same ruler: ~4.5% -> ~7% (presses at 5.7, 10.4 s). The jacket filling ~50% more of the view = the torso comes toward the eye. Identical pattern every press, both clips.
