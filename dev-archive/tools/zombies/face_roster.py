...
# (key, source face folder, tint, head SHAPE, one-line note for the ledger)
# The shape is a recipe name from head_shapes.py -- a proportional deformation of the source head's
# mesh (jaw, brow, nose, skull, cheeks, chin). Round-tripping a head through RE Mesh Editor was
# proven lossless on all 8 usable heads 2026-09-12: vertex count, every bone weight, both UV layers
# and the whole skeleton come back identical, and the recipes move NOTHING at or below the neck bone,
# so the head cannot detach from the body. Use "stock" to leave a head's shape alone.
ROSTER = [
    # --- the seven empty ENUM slots first: these work whatever happens with string keys ---
    ("FACE08", "Face00", dict(hue=0.045, sat=0.72, value=0.92), "broad", "Face00, sallow and drained"),
    ("FACE09", "Face01", dict(hue=0.030, sat=0.60, value=0.80), "gaunt", "Face01, older kill, darker"),
    ("FACE12", "Face02", dict(hue=0.055, sat=0.55, value=0.98), "heavy_jaw", "Face02, green-grey, waxy"),
    ("FACE13", "Face04", dict(hue=0.015, sat=0.90, value=0.88, ruddy=0.18), "long_skull", "Face04, fresh, still flushed"),
    ("FACE74", "Face05", dict(hue=0.060, sat=0.50, value=0.75), "big_nose", "Face05, long dead"),
    ("FACE75", "Face06", dict(hue=0.025, sat=0.78, value=1.05), "snub_nose", "Face06, bloated and pale"),
    ("FACE76", "Face07", dict(hue=0.040, sat=0.65, value=0.85), "brow_ridge", "Face07, mid-decay"),
    # --- beyond the enum: proves the string lookup, and is where the numbers come from ---
    ("FACE20", "Face00", dict(hue=0.020, sat=0.85, value=1.06, ruddy=0.22), "hollow", "Face00, fresh kill"),
    ("FACE21", "Face00", dict(hue=0.065, sat=0.48, value=0.78, grey=0.10), "bloated", "Face00, grey and greying"),
    ("FACE22", "Face01", dict(hue=0.050, sat=0.62, value=1.02), "lantern", "Face01, pallid"),
    ("FACE23", "Face01", dict(hue=0.010, sat=0.95, value=0.82, ruddy=0.25), "pug", "Face01, ruddy and dark"),
    ("FACE24", "Face02", dict(hue=0.020, sat=0.80, value=0.86), "weak_jaw", "Face02, dulled"),
    ("FACE25", "Face02", dict(hue=0.070, sat=0.45, value=0.72, grey=0.12), "flat_skull", "Face02, cadaverous"),
    ("FACE26", "Face03", dict(hue=0.035, sat=0.70, value=0.95), "small_head", "Face03, sallow"),
    ("FACE27", "Face03", dict(hue=0.015, sat=0.88, value=1.08, ruddy=0.20), "big_head", "Face03, flushed, bloated"),
    ("FACE28", "Face04", dict(hue=0.058, sat=0.52, value=0.80), "broad", "Face04, green-grey"),
    ("FACE29", "Face04", dict(hue=0.030, sat=0.75, value=1.00, grey=0.08), "gaunt", "Face04, greying hair"),
    ("FACE30", "Face05", dict(hue=0.022, sat=0.86, value=0.90, ruddy=0.15), "heavy_jaw", "Face05, recent"),
    ("FACE31", "Face05", dict(hue=0.068, sat=0.47, value=1.04), "long_skull", "Face05, waxy and drained"),
    ("FACE32", "Face06", dict(hue=0.048, sat=0.58, value=0.76), "big_nose", "Face06, dark and dead"),
    ("FACE33", "Face06", dict(hue=0.012, sat=0.92, value=0.98, ruddy=0.24), "snub_nose", "Face06, fresh"),
    ("FACE34", "Face07", dict(hue=0.062, sat=0.50, value=1.00, grey=0.10), "brow_ridge", "Face07, grey"),
    ("FACE35", "Face07", dict(hue=0.028, sat=0.82, value=0.84), "hollow", "Face07, dulled and dark"),
    ("FACE36", "Face03", dict(hue=0.072, sat=0.44, value=1.02, grey=0.10), "bloated", "Face03, waxy and greying"),
    ("FACE37", "Face06", dict(hue=0.018, sat=0.90, value=0.86, ruddy=0.20), "lantern", "Face06, fresh and dark"),
]
...
def all_face_keys():
    ...
    return SHIPPED_EVERYDAY + [k for k, _s, _t, _sh, _n in ROSTER]
...
if __name__ == "__main__":
    print("%d new faces, from %d source heads, %d distinct shapes" % (len(ROSTER), len({s for _k, s, _t, _sh, _n in ROSTER}), len({sh for _k, _s, _t, sh, _n in ROSTER})))
    print("total pool after the build: %d" % len(all_face_keys()))
    for k, src, tint, shape, note in ROSTER:
        print("  %-8s <- %-8s %-12s %-42s %s" % (k, src, shape, note, tint))
