...
albedo, so hd_hands_render.py can show WHERE on the hand the lines land.
  blender -b --python hd_hands_debug_render.py -- <anatomy out folder> <debug out folder>
"""
import bpy, sys, os
import numpy as np
argv = sys.argv[sys.argv.index("--") + 1:]; SRC, OUT = argv[0], argv[1]; os.makedirs(OUT, exist_ok=True)
def load(path):
...
alb = load(os.path.join(SRC, "pl3000_Body_ALBM.png")); m = load(os.path.join(SRC, "anat_mask.png"))
out = alb.copy()
...
out[..., 3] = 1
save(out, os.path.join(OUT, "pl3000_Body_ALBM.png"))
# flat normal so only the colours show
n = np.zeros_like(alb); n[..., 0] = 0.5; n[..., 1] = 0.5; n[..., 2] = 1.0; n[..., 3] = 1.0
save(n, os.path.join(OUT, "pl3000_Body_NRMR.png")); print("DONE")
